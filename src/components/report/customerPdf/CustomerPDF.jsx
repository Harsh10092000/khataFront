    ...
    <div className="p-3">
      ...
      <div className="border border-slate-300 p-4 h-full" ref={pdfRef}>
        <div className="flex justify-between">
          ...
          <div className="text-right text-2xl">Customer Report</div>
        </div>
            ...
            <tbody>
              {props.data.map((item, index) => (
                <tr className="bg-white border-b " key={index}>
                  ...
                  <td className="px-6 py-4">{item.cust_name}</td>
                  <td className="px-6 py-4">
                    {" "}
                    ₹ {item.tran_pay ? item.tran_pay : "0"}
                  </td>
                  <td className="px-6 py-4">
                    {" "}
                    ₹ {item.tran_receive ? item.tran_receive : "0"}
                  </td>
                </tr>
              ))}
            </tbody>
